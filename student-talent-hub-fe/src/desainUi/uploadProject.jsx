...
export default function UploadProject() {
  ...
  // State disesuaikan dengan schema ProjectCreate di FastAPI backend
  const [formData, setFormData] = useState({
    title: "",
    description: "",
    github_link: "",
    figma_link: "",
    is_open: true, // True = Open for Collaboration, False = Closed
    visibility: "Published", // Mockup UI state
  });

  const handleChange = (e) => {
    const { name, value } = e.target;
    // Handle konversi string ke boolean untuk select is_open
    if (name === "is_open") {
  ...
  const handleFileUpload = (e) => {
    e.preventDefault();
    // TODO: Implementasi upload file/gambar ke AWS S3, Cloudinary, atau Supabase Storage
    console.log("Area upload file diklik atau file di-drop");
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    // TODO: Panggil API POST /projects/
    console.log("Submitting Project Data:", formData);

    // Asumsi sukses, kembali ke halaman My Projects
    navigate("/projects");
  };
    ...
    <div className='max-w-6xl mx-auto pb-12 font-sans'>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className='flex justify-end mt-10'>
          <button
            type='submit'
            className='flex items-center gap-2 px-8 py-3.5 rounded-lg text-sm font-bold text-white bg-[#0a4b39] hover:bg-[#083a2c] shadow-lg shadow-[#0a4b39]/30 transition-all hover:-translate-y-0.5'
          >
            <svg
              className='w-5 h-5'
              fill='none'
