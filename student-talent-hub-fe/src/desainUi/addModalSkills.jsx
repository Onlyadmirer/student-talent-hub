...
export default function AddSkillModal({ isOpen, onClose }) {
  ...
  const [proficiency, setProficiency] = useState("Beginner"); // Default value

  // Jika modal sedang ditutup, jangan render apapun (return null)
  if (!isOpen) return null;

  // Mockup Master Data Keahlian (Nantinya di-fetch dari GET /skills/categories)
  const masterSkills = [
    { id: "1", name: "React.js" },
    { id: "2", name: "Next.js" },
    { id: "3", name: "Go (Golang)" },
  ...
  const handleSave = (e) => {
    ...
    // Payload yang siap dikirim ke POST /skills/user-skills
    const payload = {
      skill_id: selectedSkill,
      proficiency_level: proficiency.toLowerCase(), // 'beginner', 'intermediate', 'expert'
    };

    console.log("Menyimpan skill:", payload);
    // TODO: Panggil API authService/api di sini
      ...
      <div className='bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200'>
        ...
        <form onSubmit={handleSave}>
          <div className='p-6 space-y-6'>
            ...
            {/* Field: Proficiency Level (Segmented Control) */}
            <div className='space-y-2'>
              <label className='text-xs font-bold text-slate-500 uppercase tracking-wider'>
                Proficiency Level
              </label>
          ...
          <div className='flex justify-end items-center gap-4 px-6 py-4 bg-slate-50/50 border-t border-slate-100'>
            ...
            <button
              type='submit'
              className='px-6 py-2.5 rounded-lg text-sm font-bold text-white bg-[#0a4b39] hover:bg-[#083a2c] shadow-md shadow-[#0a4b39]/20 transition-colors'
            >
              Save Skill
            </button>
          </div>
